...
Changing the input or output device while in a voice channel SHALL take effect immediately without requiring the user to leave and rejoin. The hot-swap SHALL be performed off the asyncio event loop thread so that the UI remains responsive and audio is not disrupted during the transition.
...
#### Scenario: UI remains responsive during device restart
- **WHEN** a device change is applied while in a voice channel
- **THEN** the Textual event loop SHALL NOT be blocked by sounddevice stream open/close operations
- **THEN** the settings label SHALL update immediately upon selection (before the stream restarts)

#### Scenario: Audio is not disrupted during device restart
- **WHEN** a device change is applied while in a voice channel
- **THEN** the WebRTC connection SHALL remain established throughout the restart
- **THEN** at most a brief silence (≤ 500 ms) SHALL be audible during the transition

---

### Requirement: DeviceSelectScreen presents available devices
The `DeviceSelectScreen` modal SHALL enumerate available audio devices using `sd.query_devices()` and present them in a navigable list. "System Default" SHALL always be the first entry. Device enumeration SHALL run off the event loop thread so the picker opens immediately without freezing the UI.
